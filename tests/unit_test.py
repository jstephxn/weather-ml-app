...
class TestUnit(unittest.TestCase):
    ...
    def test_post_missing_field(self):
        ...
        # When a field is missing, app should return 200 but show an error message
        self.assertEqual(response.status_code, 200)
        html_text = response.data.decode('utf-8').lower()
        self.assertIn('error processing input', html_text)

    # Complete this function to test that the model can be loaded correctly
    def test_model_can_be_loaded(self):
        model = load_model()
        # Model object should not be None and must have predict method
